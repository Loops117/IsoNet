  ...
  const [profileResult, socialResult, subscriptionResult, notesResult, reviewsResult, disputesResult] =
    await Promise.all([
      supabase
        ...
        .select(
          "user_id, owner_name, company_name, website_url, address, phone_number, email, account_status, badge_url, company_logo_url, average_rating, review_count, start_date",
        )
        .eq("user_id", vendorId)
        .maybeSingle(),
      supabase
